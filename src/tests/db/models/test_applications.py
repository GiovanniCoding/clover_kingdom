from src.app.db.models.students import StudentRepository, Student
from src.app.db.models.applications import ApplicationRepository, Application
...
def test_create_application(db_session):
    ...
    db_application = db_session.query(Application).filter_by(student_id=application.student.id).one_or_none()
...
def test_get_applications(db_session):
    ...
    count_found = sum(1 for db_application in db_applications if db_application.id in [application.id for application in applications])
...
def test_get_approved_applications(db_session):
    ...
    student_repository.create_student(**student.__dict__)
    db_student = db_session.query(Student).filter_by(identification=student.identification).one_or_none()
    new_application = application_repository.create_application(student_id=db_student.id)
    new_application = application_repository.update_application(new_application, "Aprobada")
    approved_applications = application_repository.get_approved_applications()
    approved_applications = [application for application in approved_applications if application.id == new_application.id]
...
def test_update_application(db_session):
    ...
    db_student = db_session.query(Student).filter_by(identification=student.identification).one_or_none()
    application = application_repository.create_application(student_id=db_student.id)
    updated_application = application_repository.update_application(application, "Aprobada")
...
def test_delete_application(db_session):
    ...
    assert db_session.query(Application).filter_by(id=deleted_application.id).first().deleted_at is not None
